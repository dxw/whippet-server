...
if (!file_exists($options['mime-file'])) {
    ...
    if (!file_exists($options['mime-file'])) {
        \Whippet\LauncherFunctions::die_with_error("Unable to find file {$options['mime-file']}, and failed to load fallback", "You can obtain the most recent mime file here:\n\n  http://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types\n\nInstruct Whippet to use this file using the --mime-file argument, or save it to /etc/mime.types");
    } else {
...
if (WPS_LOCATION == 'wp-content' && !file_exists("{$options['wordpresses']}/{$options['wp-version']}")) {
    ...
    if (strtolower($choice) != 'n') {
        ...
        if (!file_exists("{$options['wordpresses']}/{$options['wp-version']}")) {
            \Whippet\LauncherFunctions::die_with_error('Tried to download WordPress, but something went wrong.', "Please download the version of WordPress you'd like to use, and extract it to {$options['wordpresses']}/<version>");
        }
    } else {
        exit(1);
    }
...
function signal_handler($signal)
{
    ...
    if (WPS_LOCATION == 'root') {
        if (file_exists(dirname($options['wp-config']).'/wp-config-original.whippet.bak')) {
            ...
            if (WPS_LOCATION == 'root') {
                \Whippet\Whippet::message(\Whippet\Colours::fg('red').'Error: '.\Whippet\Colours::fg('white').'Unable to find wp-config backup file; could not restore original configuration', 'Your wp-config file should have been backed up at '.dirname($options['wp-config'])."/wp-config-original.whippet.bak, but\n"."it is missing or unreadable. You should edit your wp-config.php by hand to remove the\n"."Whippet sections.\n");
            }
        }
    } elseif (WPS_LOCATION == 'wp-content') {
...
if (WPS_LOCATION == 'root') {
    ...
    } elseif (file_exists($options['wp-root'].'/../wp-config.php')) {
        $options['wp-config'] = $options['wp-root'].'/../wp-config.php';
    } else {
        \Whippet\LauncherFunctions::die_with_error(
