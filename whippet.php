...
EOT;
  ...
  if(!file_exists($options['config'])) {
    echo 
      ...
      "Couldn't find a configuration file at " . $options['config'] . "\n" . 
    ...
    if(strtolower($choice) != 'n') {
      file_put_contents("{$options['config']}", $skeleton_whippet_wpconfig);

      if(getenv("VISUAL")) {
        system("\$VISUAL {$options['config']} > `tty`");
      }
      else if(getenv("EDITOR")) {
        system("\$EDITOR {$options['config']} > `tty`");
      }
      else {
        echo 
          "Created settings file at {$options['config']}, but could\n" .
